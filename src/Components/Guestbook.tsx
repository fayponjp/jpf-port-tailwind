...
export default function Guestbook({ isVisible, toggle }: { isVisible: boolean, toggle: () => void }) {
    ...
    return (
        <>
            <div onClick={toggle} className={`fixed inset-0 bg-black/40 z-10 ${!isVisible ? 'hidden' : ''}`}></div>
            <div
                className={`absolute inset-[unset] right-5 mt-12 max-h-[75vh] w-180 max-w-[90vw] z-20 rounded-lg bg-white shadow-2xl lg:right-18 ${!isVisible ? 'hidden' : ''}`}
            >
